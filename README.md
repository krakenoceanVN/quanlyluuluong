...
| Method | Path | Mô tả |
|---|---|---|
| POST | `/auth/login` | đăng nhập, trả access+refresh (rate‑limit 10/60s/IP) |
| POST | `/auth/refresh` | làm mới token (rate‑limit 10/60s/IP) |
| GET | `/auth/me` | thông tin user hiện tại |
| GET/POST/PATCH/DELETE | `/users` | quản lý người dùng — **chỉ ADMIN** (tạo/sửa vai trò·reset mật khẩu/xóa) |
...
| GET | `/dashboard?date=` | dữ liệu trang chủ (昨日总量 + 今日实时) |
| GET | `/traffic?from=&to=&linkId=&adKeyword=` | truy vấn lưu lượng (kèm tổng **kỳ hiện tại vs kỳ lùi 1 ngày**) |
| GET | `/audit-logs?module=&from=&to=&userId=&page=` | nhật ký thao tác — **chỉ ADMIN** |

Chuẩn response: `{ code, message, data }`; lỗi validate trả **422** kèm `fields`; vượt rate‑limit → **429**; body > 1MB → **413**.

## Bảo mật & phân quyền

- **Vai trò**: `ADMIN` = toàn quyền (gồm quản lý người dùng + xem nhật ký); `OPERATOR` = mọi thao tác nghiệp vụ trừ `/users` và `/audit-logs` (chặn bằng `RolesGuard`).
- **JWT**: ở `NODE_ENV=production`, app **từ chối khởi động** nếu `JWT_ACCESS_SECRET`/`JWT_REFRESH_SECRET` trống hoặc còn giá trị mẫu (`change-me`/`CHANGE_ME`).
- **Rate limit** (`@nestjs/throttler`): mặc định 200/60s/IP · auth 10/60s · engine 600/60s.
- **Chống XSS**: mã tracker chỉ giữ thẻ `<script src="//…">` từ nguồn http(s), loại bỏ script inline; `targetUrl` được encode đúng ngữ cảnh.
- **Ổn định**: engine vẫn `302` khi Redis tạm sập; `graceful shutdown` (flush counter trước khi tắt); validate ngày, tên (chặn `< >`/ký tự điều khiển), giới hạn độ dài input.

## Giao diện (7 trang)

首页 (dashboard realtime 5s) · 数据查询 (lọc + biểu đồ + kỳ hiện tại/kỳ trước) · 链接管理 (inline edit, sửa, popup tạo) ·
链接编辑 (transfer 2 cột, sửa theo lô + validate) · 广告管理 · 统计管理 (2 cột) · **用户管理 (chỉ ADMIN)** · 操作日志.
...
`DATABASE_URL`, `REDIS_URL`, `JWT_ACCESS_SECRET`, `JWT_REFRESH_SECRET` (production phải là secret mạnh, không để mẫu),
`PUBLIC_LINK_DOMAIN` (không hard-code domain), `TRAFFIC_SYNC_INTERVAL_SEC`, `LINK_CACHE_TTL_SEC`,
`ADMIN_USERNAME`/`ADMIN_PASSWORD` (tài khoản admin khởi tạo khi seed),
`BUSINESS_UTC_OFFSET_MIN` (chốt "ngày" thống kê theo múi giờ nghiệp vụ — **480 = UTC+8 giờ Trung Quốc**, 420 = UTC+7 VN).

## Kiểm tra số liệu thống kê (ops)

Đối chiếu 昨日总量 (DB) vs 今日实时 (Redis) cho từng link theo đúng múi giờ nghiệp vụ:
```bash
docker compose exec api node prisma/verify-counts.mjs
```
