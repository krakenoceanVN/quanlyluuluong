...
describe('EngineService.serve', () => {
  ...
  beforeEach(() => {
    ...
    service = new EngineService(prisma as never, redis as never, flow as never, config as never);
  });
  ...
  it('redistributes all traffic to the remaining ad when one hits its daily limit', async () => {
    ...
    flow.getTodayMap.mockResolvedValue(
      new Map([
        ['m1', 100],
        ['m2', 0],
      ]),
    ...
    redisStore['link:cfg:abc'] = makeConfig([
      {
        linkAdId: 'm1',
        adId: 'a1',
        targetUrl: 'https://off-membership',
        weight: 50,
        dailyLimit: 0,
        status: false,
        adStatus: true,
